...
function DeckCard({
  ...
  const defaultClasses = `absolute inset-x-5 top-0 select-none overflow-hidden rounded-[14px] border border-stone-200 bg-white dark:border-white/[0.16] dark:bg-[#1D1D1A] ${
    active
...
export function SwipeDeck<T>({
  ...
  return (
    <div className={`w-full ${className}`}>
      ...
      >
        <div
          className={`absolute inset-x-0 top-0 bottom-0 overflow-visible ${
            enableMask
      ...
      {!hideControls && (
        <div className="mt-3 flex items-center justify-center relative z-20">
          {hideActionButtons ? (
            <div className="flex items-center gap-2.5 font-mono text-[11px] tabular-nums text-stone-500 dark:text-stone-400">
              ...
              <button
                ...
                onClick={deck.undo}
                inert={!deck.canUndo}
                className={`inline-flex items-center gap-1 rounded-[6px] px-2 py-0.5 text-[#252825] font-sans text-[11.5px] font-medium border border-[#DEE3DE] bg-white outline-none transition-all hover:bg-stone-50 active:scale-95 cursor-pointer disabled:opacity-30 disabled:cursor-not-allowed dark:border-white/[0.16] dark:bg-[#1D1D1A] dark:text-stone-200 ${spent(
                  !deck.canUndo,
                )}`}
              >
                {ICON_UNDO}
            ...
            <div className="grid h-8 w-full grid-cols-[1fr_auto_1fr] items-center gap-3">
              <button
                type="button"
